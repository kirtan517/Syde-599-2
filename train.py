...
def train(train_loader,test_loader,model,loss_function,optimizer,trail):
    ...
    with tqdm(total=EPOCHS, desc='Training') as epoch_bar:
        for epoch in range(EPOCHS):
            ...
            trail.report(np.sum(np.array(test_accuracy_epoch)) / len(test_loader.dataset), epoch)
...
def objective(trail):
    params = {
        ...
        "Number of Convolutional layers": trail.suggest_int("numberOfConvolutionLayers", 1, 4),
        "Learning Rate": trail.suggest_loguniform("learningRate", 1e-8, 1e-4),
